# 6-28-2015
# code for public paper
# update June 28, 2015
# update May 2end, 2016
# @author: Mengdi Wang
## this is the main function to train W for H_raw.
...
# This function return the List W_train

W_train_nonbinary <- function(W_matrix, K, X, similarity, disimilarity, r1, r2, beta, a, alpha, M, N,metricMatrix){
  i=1
  thro =10
  ...
  while(thro>0.001){
    ...
    W_new_matrix = W_matrix-alpha*L_W_P(W_matrix, K, X, similarity, disimilarity, r1, r2, beta, a, M, N,metricMatrix)
...
L_W_P <- function(W_matrix, K, XF, similarity, disimilarity, r1, r2, beta, a, M, N,metricMatrix){
  ...
  if(N > 0){
    ...
    M_W <- mclapply(similarity, function(x, XF, W, H,H_matrix, r,mFirstTerm,WX_list,metricMatrix) 
      Lij_W_objective_P_nonbinary(x, XF, W, H,H_matrix, r,mFirstTerm, WX_list,metricMatrix), XF = XF, W = W, H = H,H_matrix= H_matrix, r = 1,
      mFirstTerm = mFirstTerm, WX_list = WX_list, metricMatrix=metricMatrix,mc.cores=6)    
    M_W <- Reduce("+", M_W)
  }
  
  if(M > 0){
    L_W <- mclapply(disimilarity, function(x, XF, W, H,H_matrix, r,mFirstTerm,WX_list,metricMatrix) 
      Lij_W_objective_P_nonbinary(x, XF, W, H,H_matrix, r,mFirstTerm, WX_list,metricMatrix), XF = XF, W = W, H = H,H_matrix= H_matrix, r = -1,
      mFirstTerm = mFirstTerm, WX_list = WX_list,metricMatrix=metricMatrix,mc.cores=6)    
    L_W <- Reduce("+", L_W)
  }
