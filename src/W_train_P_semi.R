...
# update May 2nd, 2016
# @author: Mengdi Wang
## this is the main function to train W + regularization part
...
W_train_P_semi <- function(W_matrix, K, X, similarity, disimilarity,trainSimi, r1, r2, r3, beta, a,alpha, M, N,N_trainSimi){
  ...
  while(thro>0.001){
    print(i)
    #print(W_matrix)
    W_new_matrix = W_matrix-alpha*L_W_P_semi(W_matrix, K, X, similarity, disimilarity,trainSimi, r1, r2, r3, beta, a, M, N,N_trainSimi)
...
L_W_P_semi <- function(W_matrix, K, XF, similarity, disimilarity, trainSimi, r1, r2, r3, beta, a, M, N,N_trainSimi ){
...
#               Lij_W_objective_P(x, XF, W, H,H_matrix, r,mFirstTerm, WX_list), XF = XF, W = W, H = H,H_matrix= H_matrix, r = 1,mFirstTerm = mFirstTerm, WX_list = WX_list) 
 ...
 # for disimilarity pairs in semi-supervised setting
  if(M > 0){
  ...
  #
 # for similarity pairs in semi-supervised setting 
  if(N_trainSimi>0){
    R_semi <- mclapply(trainSimi, function(x, XF, W, H,H_matrix, r,mFirstTerm,WX_list) 
      Lij_W_objective_P(x, XF, W, H,H_matrix, r,mFirstTerm, WX_list), XF = XF, W = W, H = H,H_matrix= H_matrix, r = 1,mFirstTerm = mFirstTerm, WX_list = WX_list,mc.cores=6)    
    R_semi <- Reduce("+", R_semi)
  }
  ...
  L_W = M_W + beta*L_W+R_W_matrix+r3*R_semi
